# Requisitos Aplicacion a desaroollar

## Entregables Proyecto
- Aplicación
- Codigo fuente
- Documentacion
- Manual de despliegue

## Tipos de entidades:
- Se queda quieta.
- Se mueve cada instante a una casilla adyacente de manera aleatoria para arriba, abajo, derecha e izquierda.
- Se mantiene en su casilla y cada instante tiene una probabilidad que elegimos nosotros para moverse a una casilla de la cruceta.
- Si ya hay un color en la casilla, prevalece el color que ya existía.

## Decisiones de tecnología:
- Framework: Spring Boot, porque nos permite crear la API REST de forma rápida y ordenada. Nos ahorra mucha configuración y facilita exponer endpoints para que la web consuma el servicio.
- Lenguaje: Java 17 (el equipo está familiarizado con java y ahorra tiempo de formación)
- Tipo API: REST JSON
- Build tool: Maven (por la gestión de dependencias que conoce el equipo)

## Reuniones y comunicacion:
- La comunicacion se realizara en presencial en el laboratorio y en caso de no poder asistir presencialmente a traves de WhatsApp.
- Se realizara una reunion despues de cada hito.

## Tiempo accesible:
- 7 semanas (3 horas por semana) \implies 63 horas
- Hito 1: Encontrar con qué lo vamos a hacer (09/04/2026)
- Hito 2: Crear un servicio (16/04/2026)
- Hito 3: Desarollar test para la aplicación
- Hito 4: Desarrollar la aplicación
- Hito 5: El servicio funcione en el servidor
- Hito 6: El trabajo funcione en el servicio

## Riesgos:
- Riesgo 1: Fallo o perdida de algun miebro
- Riesgo 2: Fallo de tecnologias

## Mitigaciones:
- Mitigacion riesgo 1: Realizar reunion para reasignar las tareas.
- Mitigacion riesgo 2: Reunion para valorar si merece la pena migrar o abandonar el proyecto.
  
